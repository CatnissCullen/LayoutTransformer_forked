        ...
        for batch_idx, (input_token, segment_label, token_type, \
            ...
            if phase == 'train':
                output_cats, output_pos, output_shape = self.model(
                    input_token, input_id, segment_label, token_type, src_mask, trg_input_cats_id, trg_input_pos,
                    trg_input_shape, trg_mask)
            else:
            ...
            log_probs_cats = log_probs_cats.reshape(log_probs_cats.size(0) * log_probs_cats.size(1),
                                                    log_probs_cats.size(2))
            log_probs_pos = log_probs_pos.reshape(log_probs_pos.size(0) * log_probs_pos.size(1), log_probs_pos.size(2))
            log_probs_shape = log_probs_shape.reshape(log_probs_shape.size(0) * log_probs_shape.size(1),
                                                      log_probs_shape.size(2))
            ...
            if batch_idx % (len(dataloader) // 4) == 0:
                print('[%d/%d] Loss: %.4f Loss_cat: %.4f Loss_pos: %.4f Loss_shape: %.4f' % \
                      (batch_idx + 1, len(dataloader), loss.item(), cats_loss.item(), pos_loss.item(),
                       shape_loss.item()))

...
class Inference_SNP:
    def __init__(self, save_dir, vocab_dict, device):
        self.save_dir = save_dir  # 'test_one/'
        self.device = device
        self.vocab_dict = vocab_dict
        self.auto_filter = 10

    def run_single(self, T2B_model: Rel2Bbox, single_data: SNPSingleData):
        """
        Generate one layout
        Args:
            T2B_model: the Rel2Bbox model
            single_data: a custom VGmsdnRelDataset
        Returns: bbox coordinates of predicted layout
        """
        # SET SAVING DIR.
        log_file_name = os.path.join(self.save_dir, 'logs', single_data.scene_nm + '_infer_log.txt')
        json_file_name = os.path.join(self.save_dir, single_data.scene_nm + '_sentence.json')

        # SET MODEL
        T2B_model.to(self.device)
        T2B_model.eval()

        # SET DATA: (sentence, word_ids, segment_label, token_type)
        input_token = single_data[0].unsqueeze(0).to(self.device)  # for word embedding (global idx)
        input_obj_id = single_data[1].unsqueeze(0).to(
            self.device)  # for object id embedding  (same name different instance => different embedding)
        segment_label = single_data[5].unsqueeze(0).to(self.device)  # for sentence id embedding
        token_type = single_data[6].unsqueeze(0).to(self.device)  # for part-of-pair embedding (sbj/ rel/ obj)
        # ['[PAD]', '[CLS]', '[SEP]', '[MASK]']
        src_mask = (input_token != 0).unsqueeze(1).to(self.device)  # omit PAD (idx=0)
        global_mask = input_token >= 2  # omit PAD & CLS

        with torch.no_grad():
            start = time.time()
            # coarse BBox is generated here!!!
            vocab_logits, obj_id_logits, token_type_logits, output_box, _, refine_box, _ = \
                T2B_model(input_token, input_obj_id, segment_label, token_type, src_mask, inference=True, epoch=0,
                          global_mask=global_mask)
            end = time.time()
        print("Elapsed time,", end - start)
        print("Batch Size", input_token.size(0))

        pred_vocab = vocab_logits.argmax(2)  # word
        pred_id = obj_id_logits.argmax(2)  # object id
        # Paste back pred. masked word
        ...
        if self.auto_filter < len(output_boxes.squeeze(0)):
            return 0
        output_boxes = self.xcycwh2xyxy(output_boxes, single_data.scene_W)
        refine_boxes = self.xcycwh2xyxy(refine_boxes, single_data.scene_H)
        ...
        scene_shape = [single_data.scene_W, single_data.scene_H, 3]  # [W, H, C=3]
        self.draw_img(image_size=scene_shape, boxes=output_boxes.squeeze(0),
                      labels=pred_classes, label_ids=output_class_ids.squeeze(0),
                      save_dir=self.save_dir, name=single_data.scene_nm)  # coarse
        self.draw_img(image_size=scene_shape, boxes=refine_boxes.squeeze(0),
                      labels=pred_classes, label_ids=output_class_ids.squeeze(0),
                      save_dir=self.save_dir, name=single_data.scene_nm, mode='r')  # refined
        ...
        self.write_json(output_sentence, input_obj_id_list, json_file_name,
                        name=single_data.scene_nm)
        self.save_layout(boxes=output_boxes.squeeze(0), objs=pred_classes,
                         save_path=self.save_dir, label_ids=output_class_ids.squeeze(0),
                         name=single_data.scene_nm, image_wh=scene_shape)

    def draw_img(self, image_size, boxes, labels, label_ids, save_dir, name, mode='c'):
        # setting color
        ...
        self.show_and_save(image,
                           os.path.join(save_dir, str(name) + '_{}_{}'.format(mode,
                                                                              len(boxes)) + '.png'))
    ...
    def write_json(self, sentence, class_ids, log_file_name, name=None):
        out_dict = dict()
        out_dict['image_nm'] = name
...
class Inference_VG_MSDN():
    ...
    # TODO: find ① how to generate SG ② where the bbox come out
    def check_from_model(self, dataset_idx, dataset, model, random=False, layout_save=None):
        ...
        input_token = single_data[0].unsqueeze(0).to(self.device)  # word embedding (same name same embedding)
        input_obj_id = single_data[1].unsqueeze(0).to(
            self.device)  # object ID embedding (same name different instance => different embedding)
        segment_label = single_data[5].unsqueeze(0).to(self.device)  # sentence ID embedding
        token_type = single_data[6].unsqueeze(0).to(self.device)  # part-of-pair embedding (sbj/ rel/ obj)
        src_mask = (input_token != 0).unsqueeze(1).to(self.device)
        global_mask = input_token >= 2

        """ Predict Masked Word info & BBox & Refined BBox """
        # TODO: why???
        ...
        with torch.no_grad():
            start = time.time()
            # BBox is generated here!!!
            vocab_logits, obj_id_logits, token_type_logits, output_box, _, refine_box, _ = \
                model(input_token, input_obj_id, segment_label, token_type, src_mask, inference=True, epoch=0,
                      global_mask=global_mask)
            end = time.time()
        print("Elapsed time,", end - start)
        print("Batch Size", input_token.size(0))
            ...
            if cfg['TEST']['RANDOM']:
                ...
                if cfg['TEST']['SAMPLE_IDX'] == -1:
                    ...
                    for idx in tqdm(range(10)):
                        print('\n', idx)
                        self.check_from_model(idx, dataset=dataset, model=model, random=False,
                                              layout_save=layout_path)
                else:  # if input idx specified, run this
...
if __name__ == '__main__':
    ...
    test_output = 'saved/vg_msdn_F_seq2seq_v24/test'
    # cfg_path = './configs/vg_msdn/vg_msdn_seq2seq_v11.yaml'
    cfg_path = './configs/vg_msdn/default.yaml'
    ...
    model_path = 'saved/vg_msdn_F_seq2seq_v24/checkpoint_50_0.16316922369277578.pth'
    ...
    da = ['bush', 'kite', 'pant', 'laptop', 'paper', 'shoe', 'chair', 'ground', 'tire', 'cup', 'sky', 'bench', 'window',
          'bike', 'board', 'orange', 'hat', 'hill', 'plate', 'woman', 'handle', 'animal', 'food', 'bear', 'wave',
          'giraffe', 'background', 'desk', 'foot', 'shadow', 'lady', 'shelf', 'bag', 'sand', 'nose', 'rock', 'sidewalk',
          'motorcycle', 'fence', 'people', 'house', 'sign', 'hair', 'street', 'zebra', 'mirror', 'racket', 'logo',
          'girl', 'arm', 'flower', 'leaf', 'clock', 'dirt', 'boat', 'bird', 'umbrella', 'leg', 'bathroom', 'surfer',
          'water', 'sink', 'trunk', 'post', 'tower', 'box', 'boy', 'cow', 'skateboard', 'roof', 'pillow', 'road', 'ski',
          'wall', 'number', 'pole', 'table', 'cloud', 'sheep', 'horse', 'eye', 'top', 'neck', 'tail', 'vehicle',
          'banana', 'fork', 'head', 'door', 'bus', 'glass', 'train', 'child', 'line', 'ear', 'reflection', 'car',
          'tree', 'bed', 'cat', 'donut', 'cake', 'grass', 'toilet', 'player', 'airplane', 'ocean', 'glove', 'helmet',
          'shirt', 'floor', 'bowl', 'snow', 'couch', 'field', 'lamp', 'book', 'branch', 'elephant', 'tile', 'beach',
          'pizza', 'wheel', 'picture', 'plant', 'sandwich', 'mountain', 'track', 'hand', 'plane', 'stripe', 'letter',
          'skier', 'vase', 'man', 'building', 'short', 'surfboard', 'phone', 'light', 'counter', 'dog', 'face',
          'jacket', 'person', 'part', 'truck', 'bottle', 'jean', 'wing', '__in_image__', 'and', 'in_a', 'cover', 'over',
          'at', 'have', 'in', 'carry', 'rid', 'have_a', 'inside_of', 'wear_a', 'for', 'in_front_of', 'hang_on',
          'on_top_of', 'below', 'eat', 'beside', 'behind', 'above', 'under', 'on_front_of', 'lay_on', 'around', 'on_a',
          'look_at', 'sit_on', 'between', 'watch', 'wear', 'walk_on', 'be_in', 'along', 'hold', 'with', 'by',
          'stand_on', 'on', 'next_to', 'on_side_of', 'attach_to', 'of', 'inside', 'be_on', 'hang_from', 'near',
          'sit_in', 'stand_in', 'of_a']

...
from .inference_vg_msdn import Inference_VG_MSDN, Inference_SNP

logger = logging.getLogger('inference')


def build_inference_SNP(cfg, device):
    save_dir = cfg['TEST']['SAVE_DIR']
    vocab_dict_path = os.path.join(save_dir, 'all_idx_to_name.pkl')
    with open(vocab_dict_path, 'rb') as file:
        vocab_dict = pickle.load(file)
    infer = Inference_SNP(save_dir=save_dir, vocab_dict=vocab_dict, device=device)
